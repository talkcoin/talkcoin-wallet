...
{
    QString ret = QString("talkcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TAC unit input in lnReqAmount, we generate the URI with TAC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(TalkcoinUnits::format(TalkcoinUnits::TAC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
